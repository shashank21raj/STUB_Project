...
export default function Todo() {
  ...
  return (
    <>
      <div className="shadow-md h-full rounded-2xl">
        <div className="flex justify-between">
          <div className="text-4xl mx-4">ToDo</div>
          <div>
        ...
        {todos?.length > 0 &&
          todos.map((todo) => (
            <div className="flex items-center h-12 shadow-md rounded-2xl my-2 shadow-slate-300">
              ...
              <div className="flex justify-between w-full">
                {edit === todo._id ? (
                  todo.completed ? (
                    <input
                      type="text"
                      value={target ? target : todo.title}
                      className="w-full text-2xl line-through"
                      onChange={(e) => {
                  ...
                  <div className="w-full line-through text-2xl">
                    {todo.title}
                  </div>
                ) : (
                  <div className="w-full text-2xl">{todo.title}</div>
                )}
                {edit === todo._id ? (
                  <button
                    className="mx-2"
                    onClick={() => editTodo(todo._id, target)}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                  ...
                  <button className="mx-2" onClick={() => setEdit(todo._id)}>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 24 24"
